...
function ArtSlot({ path, label, compact = false }: { path: string; label: string; compact?: boolean }) {
  const image = path.includes("golden") ? "/pets/golden-growth.png" : path.includes("tabby") || path.includes("british") || path.includes("ragdoll") ? "/pets/cat-growth.png" : path.includes("/pets/") ? "/pets/exotic-collection.png" : null;
  if (image) return <div className={`asset-slot image-slot ${compact ? "compact" : ""}`}><img src={image} alt={label} /><small>{label}</small></div>;
